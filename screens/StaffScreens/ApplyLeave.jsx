...
const ApplyLeave = () => {
  ...
  const [applyLeaveLoading, setLeaveLoading] = useState(false);

  const getLeaves = useCallback(async () => {
    const response = await LeaveManagementServices.getLeaveTypes(
      token,
      appUser?.school_id,
    );
    setLeaves(response);
  }, [token, appUser?.school_id]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const response = await LeaveManagementServices.applyLeave(token, payload);
  ...
  return (
    ...
    >
      ...
      <View style={styles.applyLeaveForm}>
        ...
        />
        {leaveData?.leave_name !== 'permission' ? (
          <>
